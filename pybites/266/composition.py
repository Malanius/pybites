...
class File:
    ...
    def data(self) -> Optional[str]:
        """Returns the contents of the file if it exists, otherwise None."""
        if self.path.exists():
...
class RealClearPolitics(Site):
    ...
    def stats(self, loc: int = 0):
        ...
        biden = sum(
            [poll.Biden if isinstance(poll.Biden, float) else 0 for poll in polls]
        )
        sanders = sum(
            [poll.Sanders if isinstance(poll.Sanders, float) else 0 for poll in polls]
        )
        gabbard = sum(
            [poll.Gabbard if isinstance(poll.Gabbard, float) else 0 for poll in polls]
        )
