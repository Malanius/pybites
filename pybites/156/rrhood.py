...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    for line_num, line in enumerate(text.lower().splitlines()):
        for character_entry in characters:
            if type(character_entry) is str:
                if line.find(character_entry.lower()) != -1:
                    index[character_entry.lower()].append(line_num)
            if type(character_entry) is tuple:
                char = character_entry[0]
                for alias in character_entry:
                    if line.find(alias.lower()) != -1:
                        index[char.lower()].append(line_num)
                        break  # one match on line is enough
