...
def rent_or_stream(
    ...
    """
    rent_price_per_month = defaultdict(int)
    for movie in renting_history:
        rent_price_per_month[get_month(movie.date)] += movie.price

    return {
        month: RENT if price <= streaming_cost_per_month else STREAM
        for month, price in rent_price_per_month.items()
    }
