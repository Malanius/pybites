...
def get_workout_motd(day: str):
    ...
    """
    lowercase_schedule = {key.lower(): value for key, value in WORKOUT_SCHEDULE.items()}
    target_day = day.lower()
    try:
        if lowercase_schedule[target_day] == REST:
            return CHILL_OUT
        return TRAIN.format(lowercase_schedule[target_day])
    except KeyError:
        return INVALID_DAY
